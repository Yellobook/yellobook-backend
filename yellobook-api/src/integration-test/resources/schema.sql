...
CREATE INDEX ix_members_email
    ON members (email); -- 이메일에 대한 인덱스 (빠른 검색을 위해)
...
(
    ...
    member_id  BIGINT  NOT NULL
        CONSTRAINT fk_informs_member
            REFERENCES members (id), -- 작성자 (외래 키)
    team_id    BIGINT  NOT NULL
        CONSTRAINT fk_informs_team
            REFERENCES teams (id), -- 해당 팀 (외래 키)
...
(
    ...
    inform_id  BIGINT       NOT NULL
        CONSTRAINT fk_inform_comments_inform
            REFERENCES informs (id), -- 공지 (외래 키)
    member_id  BIGINT       NOT NULL
        CONSTRAINT fk_inform_comments_member
            REFERENCES members (id), -- 댓글 작성자 (외래 키)
...
(
    ...
    member_id  BIGINT NOT NULL
        CONSTRAINT fk_inform_mentions_member
            REFERENCES members (id), -- 언급된 사용자 (외래 키)
    created_at TIMESTAMP(6), -- 생성 시각
    updated_at TIMESTAMP(6), -- 수정 시각
...
(
    ...
    member_id  BIGINT       NOT NULL
        CONSTRAINT fk_participants_member
            REFERENCES members (id), -- 팀원 (외래 키)
    team_id    BIGINT       NOT NULL
        CONSTRAINT fk_participants_team
            REFERENCES teams (id), -- 팀 (외래 키)
...
(
    ...
    inventory_id   BIGINT      NOT NULL
        CONSTRAINT fk_products_inventory
            REFERENCES inventories (id), -- 재고 (외래 키)
...
(
    ...
    member_id    BIGINT  NOT NULL
        CONSTRAINT fk_orders_member
            REFERENCES members (id), -- 주문자 (외래 키)
    product_id   BIGINT
        CONSTRAINT fk_orders_product
            REFERENCES products (id), -- 주문한 제품 (외래 키)
    team_id      BIGINT
        CONSTRAINT fk_orders_team
            REFERENCES teams (id), -- 주문이 속한 팀 (외래 키)
...
(
    ...
    order_id   BIGINT       NOT NULL
        CONSTRAINT fk_order_comments_order
            REFERENCES orders (id), -- 해당 주문 (외래 키)
    content    VARCHAR(200) NOT NULL, -- 댓글 내용
    created_at TIMESTAMP(6), -- 생성 시각
    updated_at TIMESTAMP(6)  -- 수정 시각
);
...
-- VALUES ('사용자1', 'test1@gmail.com', 'profile1.jpg', 'USER', false, '2024-08-17 10:00:00', '2024-08-17 10:00:00'),
--        ('사용자2', 'test2@gmail.com', 'profile2.jpg', 'USER', false, '2024-08-17 10:00:00', '2024-08-17 10:00:00'),
--        ('사용자3', 'test3@gmail.com', 'profile3.jpg', 'USER', false, '2024-08-17 10:00:00', '2024-08-17 10:00:00');
