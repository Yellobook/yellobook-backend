...
@Tag(name = "MEMBER API", description = "회원 정보 조회 및 관리")
interface MemberApiDocs {
    ...
    ApiResponse<ProfileResponse> getMemberProfile(@Parameter(hidden = true) ApiMember apiMember);
    ...
    @SwaggerResponses({
            ...
            @SwaggerResponse(responseCode = "404", description = "MEMBER01"),
            @SwaggerResponse(responseCode = "400", description = "MEMBER03")
    })
    ApiResponse<?> updateNickname(@Parameter(hidden = true) ApiMember apiMember, UpdateNicknameRequest request);
    ...
    ApiResponse<?> updateBio(@Parameter(hidden = true) ApiMember apiMember, UpdateBioRequest request);

    @Operation(summary = "소속된 팀 목록 조회", description = "사용자가 소속된 팀 목록을 조회합니다.")
    @SwaggerResponses({
            @SwaggerResponse(responseCode = "200", description = "성공",
                    content = @Content(
                            mediaType = "application/json",
    ...
    ApiResponse<JoinedTeamsResponse> getMemberJoinedTeams(@Parameter(hidden = true) ApiMember apiMember);
}