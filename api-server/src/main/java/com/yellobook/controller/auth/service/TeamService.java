package com.yellobook.controller.auth.service;

import com.yellobook.TeamMemberRole;
import com.yellobook.common.vo.TeamMemberVO;
import com.yellobook.controller.auth.dto.response.InvitationResponse;
...
public class TeamService {
    ...
    public void setMemberCurrentTeam(Long memberId, Long teamId, String memberTeamRoleName) {
        redisTeamService.setMemberCurrentTeam(memberId, teamId, memberTeamRoleName);
    }
    ...
    public TeamMemberVO getCurrentTeamMember(Long memberId) {
        String key = redisTeamService.generateTeamKey(memberId);

        // 팀에대한 사용자가 존재하지 않는다면
        if (!Boolean.TRUE.equals(redisTeamService.hasKey(key))) {
            // 사용자가 참여한 팀 목록을 조회
        ...
        List<String> values = redisTeamService.getValueList(key, 0, 1);
    ...
    public String generateInvitationUrl(Long teamId, TeamMemberRole role) {
        ...
        //호스트 별로 다르게!
        //키를 도메인에 맞춰서
        redisTeamService.setValueWithExpiry(key, value, 15, TimeUnit.MINUTES);
    ...
    public InvitationResponse getInvitationInfo(String key) {
        Object value = redisTeamService.getValue("team:invite:" + key);
        if (value == null) {
